...
- 루트의 옛 설계 문서들(`PRD_*`, `Engineering_Spec_*`, `Implementation_Roadmap_*`,
  `Execution_Plan_*`, `Capture_Inbox_*`)은 **코드와 어긋나 있습니다.** 2026-09-21에
  각 문서 맨 위에 경고 헤더를 달아뒀으니 그걸 먼저 보세요. 본문은 안 고칩니다 —
  왜 그렇게 설계했다가 바뀌었는지를 남겨두는 용도입니다. **기준은 코드입니다.**
- 반대로 이 둘은 코드에 맞춰 고쳐두었으니 믿어도 됩니다.
  - `AI_Functional_Spec.md` — AI 구조와 실패 처리. 코드가 §7을 인용합니다
  - `Release_Readiness_Checklist.md` — 출시 심사 항목. 제3자 전송이 **Jina와 Gemini
    두 곳**이라는 것이 여기 있습니다. 스토어 고지를 쓸 때 빠뜨리기 쉬운 자리입니다

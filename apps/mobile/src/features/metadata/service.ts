 ...
 * 추출하기 위한 시스템 프롬프트 가이드라인입니다.
 *
 * 모델은 **Gemini 클라우드**입니다. 앱은 Supabase Edge Function `/generate`를 부르고,
 * 키와 모델 선택은 서버가 갖습니다. 여기에 'Apple Intelligence / Gemini Nano 온디바이스'라고
 * 적혀 있었는데 그런 경로는 만든 적이 없습니다. 주석이 코드와 다른 말을 하면, 다음 사람이
 * 없는 폴백을 찾아 헤매거나 스토어 고지에서 제3자 전송을 빠뜨리게 됩니다.
 *
